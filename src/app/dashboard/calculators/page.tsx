...
const calculators: Calculator[] = [
  ...
  { id: 'debt-payoff', name: 'Debt Payoff Calculator', description: 'Create debt elimination plan', category: 'financial', icon: 'debt-icon', color: 'from-red-500 to-red-600', available: true },
  { id: 'compound-interest', name: 'Compound Interest Calculator', description: 'See investment growth', category: 'financial', icon: 'compound-icon', color: 'from-purple-500 to-purple-600', available: true },
  { id: 'rent-vs-buy', name: 'Rent vs Buy / Lease vs Buy', description: 'Compare renting vs buying (house & auto)', category: 'financial', icon: 'house-icon', color: 'from-cyan-500 to-blue-600', available: true },
  { id: 'cash-vs-financing', name: 'Cash vs Financing Strategy', description: 'Optimize asset purchases with retirement accounts', category: 'financial', icon: 'wallet-icon', color: 'from-emerald-500 to-teal-600', available: true },
];
...
function CalculatorCard({ calculator }: { calculator: Calculator }) {
  const cardContent = (
    <Card className="h-full transition-all duration-300 hover:scale-105 hover:shadow-xl cursor-pointer">
      <div className="p-6 h-full flex flex-col">
        <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${calculator.color} flex items-center justify-center text-3xl mb-4 shadow-lg`}>
  ...
  return (
    <Link href={`/dashboard/calculators/${calculator.id}`}>
      {cardContent}
    </Link>
  );
}
