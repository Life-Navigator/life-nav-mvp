...
0. **URGENT — rotate the Supabase Management API access token** (`sbp_…`, pasted in chat 2026-06-16 to apply the discovery migration). It is now in the session transcript. Revoke it in Supabase → Account → Access Tokens and issue a fresh one. Highest priority of this list (a PAT grants project-management access).
