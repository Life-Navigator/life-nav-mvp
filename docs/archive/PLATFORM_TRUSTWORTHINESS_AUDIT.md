...
21. **Goals page shows 0% for everything.** `dashboard/goals/page.tsx:139,144,148` reads camelCase `progressPercentage`/`targetDate` and compares `=== 'HIGH'`/`'COMPLETED'`, but API returns snake*case `progress_percent`/`target_date` and lowercase status. Every goal renders 0%, no date, grey badge. \_Fix: read snake_case; case-insensitive compare.*
