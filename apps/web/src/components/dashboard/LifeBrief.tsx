...
export default function LifeBrief() {
  ...
  return (
    <div className="rounded-2xl border border-indigo-300/40 bg-gradient-to-br from-[#0f172a] via-[#13294b] to-[#0d3a4a] p-5 text-white shadow-md">
      ...
      <h2 className="mt-1 text-xl font-bold leading-snug text-white">{brief.headline}</h2>

      {summary && (
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-200">{summary}</p>
      )}
      ...
      {(nextMove || biggestRisk) && (
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {nextMove && (
            <div className="flex items-start gap-2 rounded-lg bg-white/[0.06] px-3 py-2 ring-1 ring-inset ring-white/10">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm leading-relaxed text-slate-100">{nextMove}</p>
              </div>
            </div>
          )}
          {biggestRisk && (
            <div className="flex items-start gap-2 rounded-lg bg-white/[0.06] px-3 py-2 ring-1 ring-inset ring-white/10">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm leading-relaxed text-slate-100">{biggestRisk}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <button
        ...
        className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-teal-300 hover:text-teal-200 hover:underline"
      >
        {expanded ? (
          <>
      ...
      {expanded && (
        <div className="mt-4 space-y-4 border-t border-white/10 pt-4">
          ...
          {brief.readiness_line && <p className="text-xs text-slate-300">{brief.readiness_line}</p>}
          ...
          {(typeof brief.confidence_pct === 'number' || brief.source) && (
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-white/10 pt-3 text-[11px] text-slate-400">
