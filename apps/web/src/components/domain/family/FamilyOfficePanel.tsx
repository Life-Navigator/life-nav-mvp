...
// Family Office panel — estate / trust / beneficiary / survivor / legacy readiness from
// GET /v1/family/office (FamilyOfficeService). Extracted from the former orphan /dashboard/family-office
// page so it can be surfaced INSIDE the Family experience (the Estate tab) instead of a non-nav route.
// Each pillar G/Y/O/R with what's in place, what's missing, the next step. Attorney boundary; not legal advice.
...
export default function FamilyOfficePanel() {
  ...
  if (loading) return <div className="text-gray-500 py-6">Assessing your family office…</div>;
  // Honest empty state: never fabricate readiness when the engine has no data.
  if (!o)
    return (
      <div className="rounded-lg border border-slate-200 bg-white p-6 text-sm text-gray-500">
        Your Family Office readiness appears once you add family details or upload a will, trust, or
        insurance document — every score is grounded in your real data.
      </div>
    );

  return (
    <div>
      <div className={`rounded-xl border-2 ${RING[o.legacy_status]} p-6 flex items-center gap-5`}>
        <div
