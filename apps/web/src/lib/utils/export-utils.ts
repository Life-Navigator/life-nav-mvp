...
export const exportToPdf = async (
  ...
  try {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text('Generated with LifeNavigator', 105, doc.internal.pageSize.height - 10, {
        align: 'center',
      });
    }
...
const exportInvestmentGrowthToPdf = (doc: any, data: any) => {
  ...
  if (scenario.contribution_amount) {
    doc.text(
      `Contribution: ${formatCurrency(scenario.contribution_amount)} per ${scenario.contribution_frequency}`,
      14,
      70
    );
    if (scenario.contribution_growth_rate) {
      doc.text(
        `Contribution Growth Rate: ${formatPercent(scenario.contribution_growth_rate)}`,
        14,
        75
      );
    }
  ...
  doc.text(
    `Compound Annual Growth Rate: ${formatPercent(summary.compound_annual_growth_rate)}`,
    14,
    110
  );
  doc.text(
    `Inflation-Adjusted Balance: ${formatCurrency(summary.inflation_adjusted_balance)}`,
    14,
    115
  );
...
const exportAssetAllocationToPdf = (doc: any, data: any) => {
  ...
  Object.entries(allocation).forEach(([asset, percentage]: [string, any]) => {
    if (percentage > 0) {
      doc.setFontSize(10);
      doc.text(
        `${asset.charAt(0).toUpperCase() + asset.slice(1)}: ${formatPercent(percentage)}`,
        14,
        yPosition
      );
      yPosition += 5;
  ...
  const tableRows = Object.entries(asset_class_metrics).map(
