/**
 * GET /api/ops/readiness?window_days=30 — Sprint R Phase 7.
 *
 * Operator-only operational-readiness snapshot for procurement +
...
import { computeReadinessSnapshot } from '@/lib/ops/readiness-queries';
...
export async function GET(request: NextRequest) {
  ...
  const window_days = Math.max(1, Math.min(90, Number.isFinite(Number(raw)) ? Number(raw) : 30));

  const snapshot = await computeReadinessSnapshot(sb, window_days);
