...
export default function RetirementPlanningPage() {
  ...
  const [healthcareProjection, setHealthcareProjection] = useState<HealthcareProjection | null>(
    null
  );
    ...
    try {
      const response = await fetch('/api/retirement/withdrawal-strategies', {
        ...
        body: JSON.stringify({
          ...
          optimize: true,
          portfolioBalance:
            accountTotals?.totalBalance || 0 /* Sprint 45B: no fabricated balance — real or 0 */,
          annualExpenses: (plan?.targetMonthlyIncome || 0) * 12 /* 45B: no fabricated income */,
          retirementYears: (plan?.lifeExpectancy || 90) - (plan?.retirementAge || 65),
          socialSecurityIncome: socialSecurity?.calculatedBenefits?.atFRA || 0 /* 45B */,
    ...
    try {
      const response = await fetch('/api/retirement/scenarios', {
        ...
        body: JSON.stringify({
          ...
          currentSavings:
            accountTotals?.totalBalance || 0 /* Sprint 45B: no fabricated balance — real or 0 */,
          monthlyContributions: accountTotals?.totalMonthlyContributions || 0 /* 45B */,
          socialSecurityBenefit: socialSecurity?.calculatedBenefits?.atFRA || 0 /* 45B */,
          annualExpenses: (plan?.targetMonthlyIncome || 0) * 12 /* 45B: no fabricated income */,
        }),
      });
      if (response.ok) {
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
            ...
            <button
              onClick={() => setActiveTab('roth-conversions')}
          ...
          <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
            <div>
              ...
              <div className="text-sm text-green-600">
                Test your plan against market crashes, inflation
              </div>
            </div>
            <button
              ...
              <div key={account.id} className="p-4 hover:bg-gray-50">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex gap-2 mt-1">
                      <span
                        className={`px-2 py-0.5 text-xs rounded-full ${account.taxStatus === 'TAX_FREE' ? 'bg-green-100 text-green-700' : account.taxStatus === 'TAXABLE' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}
                      >
                        {account.taxStatus?.replace(/_/g, ' ')}
                      </span>
                      {account.contributionRoom && account.contributionRoom > 0 && (
                        <span className="px-2 py-0.5 text-xs bg-yellow-100 text-yellow-700 rounded-full">
                          {formatCurrency(account.contributionRoom)} room
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {account.rmdAmount && account.rmdAmount > 0 && (
                  <div className="mt-2 p-2 bg-orange-50 border border-orange-200 rounded text-sm text-orange-700">
                    RMD Required: {formatCurrency(account.rmdAmount)} this year
                  </div>
                )}
              </div>
            ))
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-xl p-6 text-white">
        ...
        <p className="text-sm opacity-75">
          Optimize your tax bracket utilization through strategic Roth conversions
        </p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-xl p-4 text-white">
          ...
          <div className="text-2xl font-bold">
            {formatCurrency(healthSavingsData?.summary?.totalHSABalance || 0)}
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-200">
          {healthSavingsData?.hsaAccounts && healthSavingsData.hsaAccounts.length > 0 ? (
            healthSavingsData.hsaAccounts.map((account) => (
                  ...
                  <div className="text-right">
                    ...
                    {account.employerContributions > 0 && (
                      <div className="text-xs text-green-600">
                        +{formatCurrency(account.employerContributions)} employer
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-4">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-medium">
                      ...
                      {formatCurrency(
                        account.hdhdPlanType === 'family'
                          ? healthSavingsData?.limits?.current?.hsa?.family || 8300
                          : healthSavingsData?.limits?.current?.hsa?.individual || 4150
                      )}
                    </span>
                  </div>
                  <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, ((account.ytdContributions + account.employerContributions) / (account.hdhdPlanType === 'family' ? healthSavingsData?.limits?.current?.hsa?.family || 8300 : healthSavingsData?.limits?.current?.hsa?.individual || 4150)) * 100)}%`,
                      }}
                    />
                  </div>
                  {account.catchUpEligible && (
              ...
              >
                ...
                {fsa.carryoverFromLastYear > 0 && (
                  <div className="mt-2 text-xs text-green-600">
                    Includes {formatCurrency(fsa.carryoverFromLastYear)} carryover from last year
                  </div>
                )}
              </div>
            ))
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    >
                      {scenario.comparison.successDelta >= 0 ? '+' : ''}
                      {formatPercent(scenario.comparison.successDelta, 0)}
                    </div>
                  </div>
                  <div>
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'roth-conversions':
        return renderRothConversions();
      case 'health-savings':
        return renderHealthSavings();
      case 'healthcare':
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex justify-between items-center">
            ...
            <span
              className={`px-3 py-1 rounded-full text-sm font-medium ${getScoreColor(plan?.readinessScore || 0)}`}
            >
              Score: {plan?.readinessScore || 0}/100
            </span>
          </div>
        </div>
      </div>
