...
import { useTransactions } from '@/lib/hooks/useFinanceData';

// P0: Total Assets / Liabilities / Net Worth come from the ONE canonical finance summary — NOT a
// frontend calculation. This is the same source the resolver + finance widgets read, so Overview
// can never contradict them. Monthly cash flow stays transaction-derived (raw rows, allowed).
...
export default function OverviewPage() {
  const [summary, setSummary] = useState<CanonicalSummary | null>(null);
  ...
  const financialSummary = useMemo(() => {
    ...
    const totalLiabilities = summary?.total_debt ?? 0;
    const netWorth = summary?.net_worth ?? 0;

    // Monthly cash flow from recent transactions (raw rows — allowed exception)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        <div className="p-6 bg-white rounded-lg shadow dark:bg-slate-800">
          ...
          {loading ? (
            ...
            <p className="text-3xl font-bold">
              $
              {financialSummary.totalAssets.toLocaleString('en-US', {
        ...
        <div className="p-6 bg-white rounded-lg shadow dark:bg-slate-800">
          ...
          {loading ? (
            ...
            <p className="text-3xl font-bold">
              $
              {financialSummary.totalLiabilities.toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </p>
        ...
        <div className="p-6 bg-white rounded-lg shadow dark:bg-slate-800">
          ...
          {loading ? (
            ...
            <p className="text-3xl font-bold">
              $
              {financialSummary.netWorth.toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </p>
        ...
        <div className="p-6 bg-white rounded-lg shadow dark:bg-slate-800">
          ...
          {loading ? (
            ...
            <p
              className={`text-3xl font-bold ${financialSummary.monthlyCashFlow >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
            >
              {financialSummary.monthlyCashFlow >= 0 ? '+' : '-'}$
              {Math.abs(financialSummary.monthlyCashFlow).toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </p>
