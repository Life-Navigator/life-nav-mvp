  ...
  if (status === 'loading' || loading) {
    return (
      <div className="container mx-auto py-6 px-4 md:px-6">
        <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white mb-6">Budget</h1>
    ...
    <div className="container mx-auto py-6 px-4 md:px-6">
      ...
      {budgets.length === 0 ? (
        ...
        <div className="space-y-4">
          {budgets.map((budget) => (
            <div key={budget.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{budget.category}</h3>
              <div className="mt-2 flex justify-between text-sm">
                <span className="text-gray-600 dark:text-gray-400">Budgeted: ${budget.budgeted}</span>
                <span className="text-gray-600 dark:text-gray-400">Spent: ${budget.spent}</span>
                <span className="text-gray-600 dark:text-gray-400">Remaining: ${budget.remaining}</span>
              </div>
            </div>
          ))}
        </div>
