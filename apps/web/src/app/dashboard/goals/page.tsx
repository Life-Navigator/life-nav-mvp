  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // The API returns snake_case fields and lowercase enums; normalize to the
        // shape this page renders so goals don't all show 0% / no date / grey badges.
        const normalizedGoals = (goalsData.goals || []).map((g: any) => ({
          ...g,
          priority: (g.priority ?? '').toString().toUpperCase(),
          status: (g.status ?? '').toString().toUpperCase(),
          progressPercentage: g.progressPercentage ?? g.progress_percent ?? 0,
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg font-medium text-gray-900 dark:text-white">
                    {dashboardData.riskAssessment.status === 'COMPLETED'
                      ? 'Completed'
                      : 'In Progress'}
                  </p>
                </div>
        ...
        {!dashboardData.goals.length && !dashboardData.riskAssessment && (
          <div className="mt-8 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
              Welcome to Goals & Risk Assessment!
            </h2>
