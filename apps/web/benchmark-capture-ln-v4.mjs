...
// version, latency) so the trust analysis can use real provenance. Writes docs/advisor-benchmark/raw/lifenavigator_v4.json.
...
const OUT = new URL('../../docs/advisor-benchmark/raw/lifenavigator_v4.json', import.meta.url);
const [URL_, ANON, SERVICE] = readFileSync('/tmp/sweep_creds.txt', 'utf8').trim().split('\n').map((s) => s.trim());
const admin = createClient(URL_, SERVICE, { auth: { persistSession: false } });
const users = [];

async function mkUser(tag) {
  const email = `benchv4-${tag}-${randomUUID().slice(0, 6)}@example.com`, password = 'Bx!' + Math.random().toString(36).slice(2, 12);
...
async function run() {
  ...
  for (const sc of SCEN) {
    ...
    try {
      ...
      const cid = `benchv4-${sc.id}-${randomUUID().slice(0, 8)}`;
...
try { await run(); } catch (e) { console.error('CAPTURE ERROR:', e.stack || e.message); }
  ...
  console.log(`[cleanup] removed ${users.length} users · wrote ${out.length} records to raw/lifenavigator_v4.json`);
}
