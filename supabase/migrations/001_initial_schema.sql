-- ============================================================================
-- Life Navigator - Supabase Schema (Non-Sensitive Data Only)
-- ============================================================================
-- This schema contains ONLY non-sensitive data that can be stored in Supabase.
-- All HIPAA, PCI, and PII data MUST remain in the on-prem DGX PostgreSQL.
-- ============================================================================
...
-- ============================================================================
-- Note: Full user data (SSN, DOB, etc.) stored in DGX
CREATE TABLE public.profiles (
  id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  dgx_user_id TEXT NOT NULL UNIQUE,  -- Reference to DGX user

  -- Display preferences (non-PII)
  display_name TEXT,  -- User-chosen display name (not legal name)
  avatar_url TEXT,
...
-- ============================================================================
-- Note: Specific financial targets stored in DGX
CREATE TABLE public.goals (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  dgx_goal_id TEXT,  -- Reference to detailed goal in DGX
  ...
  -- Progress (percentage only - amounts in DGX)
...
CREATE TABLE public.achievements (
  ...
  criteria_type TEXT NOT NULL,  -- 'goal_completed', 'streak', 'level_up', etc.
...
CREATE TABLE public.feedback (
  ...
  -- Content (user should not include PII)
  title TEXT NOT NULL,
  description TEXT NOT NULL,
...
BEGIN
  ...
  VALUES (
    NEW.id,
    COALESCE(NEW.raw_user_meta_data->>'dgx_user_id', NEW.id::text),
...
-- COMMENTS
-- ============================================================================
COMMENT ON TABLE public.profiles IS 'User profiles with minimal non-PII data. Full user data in DGX.';
COMMENT ON TABLE public.goals IS 'Goal metadata only. Financial targets and amounts stored in DGX.';
COMMENT ON TABLE public.achievements IS 'Gamification achievements for user engagement.';
COMMENT ON COLUMN public.profiles.dgx_user_id IS 'Reference to user record in DGX secure database.';
COMMENT ON COLUMN public.goals.dgx_goal_id IS 'Reference to detailed goal with financial data in DGX.';
