...
async def get_career_profile(
    ...
    result = await db.execute(
        ...
        .options(selectinload(CareerProfile.experiences), selectinload(CareerProfile.skills))
    )
...
@router.post("/profile", response_model=CareerProfileResponse, status_code=status.HTTP_201_CREATED)
async def create_career_profile(
    ...
    profile = CareerProfile(**profile_data.dict(), user_id=current_user.id, tenant_id=current_user.tenant_id)
    db.add(profile)
    await db.commit()
...
@router.post("/experiences", response_model=JobExperienceResponse, status_code=status.HTTP_201_CREATED)
...
async def create_skill(
    ...
    skill = Skill(**skill_data.dict(), profile_id=profile.id, user_id=current_user.id, tenant_id=current_user.tenant_id)
    db.add(skill)
    await db.commit()
